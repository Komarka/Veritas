(() => {
  ...
  var errorBannerText = document.querySelector("#error-banner-text");
  var loadingBanner = document.querySelector("#loading-banner");
  var loadingBannerText = document.querySelector("#loading-banner-text");
  var sessionLabel = document.querySelector("#session-label");
  var claimText = document.querySelector("#claim-text");
  var factCheckBtn = document.querySelector("#fact-check-btn");
  ...
  var sourceModePanels = document.querySelectorAll("[data-source-panel]");
  var resultOnlyEls = document.querySelectorAll(".result-only");
  var checkModeButtons = document.querySelectorAll("[data-check-mode]");
  var checkModePanels = document.querySelectorAll("[data-check-panel]");
  var imageFileInput = document.querySelector("#image-file-input");
  var imageDropZone = document.querySelector("#image-drop-zone");
  var imageEmptyState = document.querySelector("#image-empty-state");
  var imagePreviewState = document.querySelector("#image-preview-state");
  var imagePreview = document.querySelector("#image-preview");
  var imageMeta = document.querySelector("#image-meta");
  var imageCheckBtn = document.querySelector("#image-check-btn");
  var imageResultBox = document.querySelector("#image-result-box");
  var imageVerdictEl = document.querySelector("#image-verdict");
  var imageScoreEl = document.querySelector("#image-score");
  var imageAiProbabilityEl = document.querySelector("#image-ai-probability");
  ...
  function base64FromDataUrl(dataUrl) {
    const [, base642 = ""] = String(dataUrl).split(",");
    return base642;
  }
  ...
  async function setSelectedImageFromUrl(imageUrl) {
    ...
    const typedBlob = blob.type ? blob : new Blob([blob], { type: contentType || "application/octet-stream" });
  ...
  function setImageCheckBusy(isBusy) {
    ...
    imageCheckBtn.innerHTML = isBusy ? "<span>Scanning image...</span>" : '<span>Scan Image</span><svg class="btn-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3l7 3v5c0 5-3 8.5-7 10-4-1.5-7-5-7-10V6l7-3z" /><path d="M9 12l2 2 4-5" /></svg>';
    if (isBusy) {
  ...
  function compactDomain(domain) {
    ...
    return cleanDomain.length > 17 ? `${cleanDomain.slice(0, 14)}...` : cleanDomain;
  }
  ...
  function fallbackChartData(sources, score) {
    ...
    const domains = sources.map((source) => sourceDomain(source)).filter((domain) => {
    ...
    const averageTrustScore = fallbackSources.length ? Math.round(
      fallbackSources.reduce((total, source) => total + source.weight, 0) / fallbackSources.length
    ) : clampScore(score);
    return {
  ...
  function normalizeChartData(payload, sources, score) {
    ...
    const rawSources = Array.isArray(chartData?.sources) ? chartData.sources : null;
    ...
    if (!normalizedSources.some((source) => source.isKeyEvidence)) {
      const strongestSource = normalizedSources.reduce(
        (best, source, index) => source.weight > normalizedSources[best].weight ? index : best,
        0
      );
    ...
    ) ? clampScore(chartData.averageTrustScore) : Math.round(
      normalizedSources.reduce((total, source) => total + source.weight, 0) / normalizedSources.length
    );
    return {
      title: typeof chartData?.title === "string" && chartData.title.trim() ? chartData.title.trim() : "",
  ...
  function normalizeResult(payload) {
    return {
      ...
      facts: typeof payload?.facts === "string" ? payload.facts : "Facts are unavailable.",
  ...
  function normalizeImageResult(payload) {
    ...
    return {
      ...
      aiProbability,
      aiAnalysis: typeof payload?.aiAnalysis === "string" ? payload.aiAnalysis : "Visual integrity analysis is unavailable.",
      textAnalysis: typeof payload?.textAnalysis === "string" ? payload.textAnalysis : "Text grounding analysis is unavailable.",
      sources: Array.isArray(payload?.sources) ? payload.sources.filter(
        (source) => typeof source === "string" && source.startsWith("http")
      ) : [],
      chartData: null
  ...
  function renderImageResult(payload) {
    ...
    scoreEl.textContent = String(Math.round(result.score));
    updateScoreGauge(result.score, color);
    confidenceChip.textContent = result.isAiGenerated ? "AI Generation Risk Detected" : confidenceLabel(result.score);
